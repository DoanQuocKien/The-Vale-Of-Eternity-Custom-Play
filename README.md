...
- **Random card generator** — generates a semantically unique card using the pure client-side RAG AI concept engine. Warns if there are unsaved changes before generating.
...
- Displays a live scrolling carousel of **5 unique card concepts** generated by a lightweight, client-side JS RAG keyword semantic matching engine (referencing `/rag_database.json`).
...
- Silently samples fresh concept batches in the background — seamless, infinite inspiration.
...
| AI card concepts | Pure client-side JS semantic keyword Jaccard overlap engine |
...
> **Note:** Native AI features (upscaling, background removal) require running via `npm run neu:dev` or the built `.exe`. They will gracefully degrade to browser fallbacks in `npm run dev`. The client-side JS RAG suggestions run fully browser-native.
...
```
├── electron/
│   └── vale_ai.py        # Python AI sidecar (background removal, upscaling)
├── public/
...
│   ├── utils/
│   │   ├── pythonRunner.js         # Neutralino sidecar bridge (upscale, remove-bg)
...
- [x] v1.4.0 — Custom Background & Emblem Designer (Family Designer), Vector Polygon tool, variable Text weight calibration mapping (`1-100` sliders), and pure offline Client-side JS RAG suggestion engine

### 🔜 v1.5.0 — Rulebook Editor
- [ ] Rulebook page editor — design custom rulebook pages with layout controls, text formatting, and printable export
